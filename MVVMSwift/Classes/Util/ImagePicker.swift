...
open class ImagePicker: NSObject {
    private let pickerController: UIImagePickerController // 피커 컨트롤러
    private weak var presentationController: UIViewController? // 현재 컨트롤러
    private weak var delegate: ImagePickerDelegate? // 델리게이트
    ...
    public func present(from sourceView: UIView) {
        ...
        if let action = self.action(for: .savedPhotosAlbum, title: NSLocalizedString("CameraRoll", comment: "")) {
            alertController.addAction(action)
        }
